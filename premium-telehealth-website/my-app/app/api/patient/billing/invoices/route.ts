...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    try {
      ...
      // ====================================================================
      {
        try {
          ...
          if (stripeCustomerId) {
            ...
            // Guard against a malformed Stripe response — `.data` should be an array.
            const stripeInvoices = {
              data: Array.isArray(stripeInvoicesResponse.data)
      ...
      // Include $0 trial invoices — Stripe generates a real, downloadable PDF
      // for the trial period and patients asked to be able to download it.
      // Previously we filtered `amount > 0`, which hid that receipt and
      // fell through to the synthetic "pending review" placeholder (which
      // has no PDF, so the Download button 404'd).
      // Guard against any non-array value from Prisma (defensive).
      const invoicesArray = Array.isArray(invoices) ? invoices : [];
      const response: InvoiceResponse[] = invoicesArray.map(invoice => ({
        id: invoice.id,
